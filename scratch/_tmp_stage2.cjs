...
const SNAP={}; SNAP.matte=Array.from(wMap);
...
SNAP.completion=Array.from(wMap);
...
SNAP.floors=Array.from(wMap);
      ...
      return { SNAP: (()=>{ const crop={x0:340,y0:540,w:400,h:340}; const out={}; for(const k in SNAP){ const a=SNAP[k]; const c=new Array(crop.w*crop.h); for(let y=0;y<crop.h;y++)for(let x=0;x<crop.w;x++) c[y*crop.w+x]=+a[(y+crop.y0)*W+(x+crop.x0)].toFixed(3); out[k]=c;} out.crop=crop; return out; })(),
    ...
    fs.writeFileSync('/tmp/claude-0/-home-user-tshirt-mockup-generator/de3bd9a4-55b7-54a1-adf3-3147e5250efd/scratchpad/stage-snap.json', JSON.stringify(r.SNAP));
