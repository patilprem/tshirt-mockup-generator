...
window.__dbg = { unk, dwarm, dfab, occluder: new Uint8Array(occluder) };
...
{ const D = window.__dbg; const rows=[]; for (let yy=558; yy<700; yy+=5) for (let xx=618; xx<764; xx+=5) { const i=yy*W+xx; if (wMap[i]<=0.15 || core[i]) continue; rows.push([xx,yy,+wMap[i].toFixed(2),+unrel[i].toFixed(2),D.unk[i],D.occluder[i],D.dwarm[i],D.dfab[i]]); } console.log("ROWS "+JSON.stringify(rows.slice(0,100))); }
